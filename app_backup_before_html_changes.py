...
def create_donut_chart(metrics: Dict[str, Any]) -> go.Figure:
    ...
    colors = [ColorScheme.PRIMARY_GREEN, ColorScheme.WARNING_ORANGE, ColorScheme.CRITICAL_RED]
    ...
    if metrics.get("date_min") and metrics.get("date_max"):
        ...
        date_range_text = f"<br><span style='font-size:14px; color:{OPTIMITIVE_COLORS['text_secondary']}'>Período: {date_min_str} - {date_max_str}</span>"
...
def create_html_report(
    ...
    def generate_html_charts(df, on_col, time_col, kpi_var):
        ...
        try:
            ...
            # Intentar usar KDE para líneas suaves
            try:
                from scipy import stats
                
                if len(off_data) > 1:
                    kde_off = stats.gaussian_kde(off_data[kpi_var].dropna())
                    x_min, x_max = df_ready[kpi_var].min(), df_ready[kpi_var].max()
                    x_range = np.linspace(x_min, x_max, 300)
                    y_off_density = kde_off(x_range)
                    y_off_count = y_off_density * len(off_data) * (x_max - x_min) / 300
                    
                    fig3.add_trace(go.Scatter(
                        x=x_range, y=y_off_count,
                        mode='lines', fill='tonexty', name='OFF',
                        line=dict(color='red', width=2),
                        fillcolor='rgba(255, 0, 0, 0.3)'
                    ))
                
                if len(on_data) > 1:
                    kde_on = stats.gaussian_kde(on_data[kpi_var].dropna())
                    y_on_density = kde_on(x_range)
                    y_on_count = y_on_density * len(on_data) * (x_max - x_min) / 300
                    
                    fig3.add_trace(go.Scatter(
                        x=x_range, y=y_on_count,
                        mode='lines', fill='tonexty', name='ON',
                        line=dict(color='green', width=2),
                        fillcolor='rgba(0, 128, 0, 0.3)'
                    ))
                    
                fig3.update_yaxis(title="Cantidad de muestras")
                    
            except ImportError:
                # Fallback a histogramas
                if len(off_data) > 0:
                    fig3.add_trace(go.Histogram(
                        x=off_data[kpi_var], name='OFF',
                        marker_color='red', opacity=0.7, histnorm=''
                    ))
                if len(on_data) > 0:
                    fig3.add_trace(go.Histogram(
                        x=on_data[kpi_var], name='ON',
                        marker_color='green', opacity=0.7, histnorm=''
                    ))
                fig3.update_yaxis(title="Frecuencia")
            
            fig3.update_layout(
                title=f"{kpi_var} - Distribución{ready_info}",
                xaxis_title=kpi_var,
                template="plotly_white",
                height=400,
                barmode='overlay'
            )
            charts_html.append(pio.to_html(fig3, include_plotlyjs=False, div_id=f"chart3_{kpi_var}"))
        except Exception as e:
        ...
        try:
            ...
            if len(off_data) > 0:
                stats_data.append({
                    'Estado': 'OFF',
                    'N° Muestras': len(off_data),
                    'Media': f"{off_data[kpi_var].mean():.4f}",
                    'Desv. Std': f"{off_data[kpi_var].std():.4f}",
                    'Mínimo': f"{off_data[kpi_var].min():.4f}",
                    'Máximo': f"{off_data[kpi_var].max():.4f}",
                    'Mediana': f"{off_data[kpi_var].median():.4f}",
                })
            
            if len(on_data) > 0:
                stats_data.append({
                    'Estado': 'ON',
                    'N° Muestras': len(on_data),
                    'Media': f"{on_data[kpi_var].mean():.4f}",
                    'Desv. Std': f"{on_data[kpi_var].std():.4f}",
                    'Mínimo': f"{on_data[kpi_var].min():.4f}",
                    'Máximo': f"{on_data[kpi_var].max():.4f}",
                    'Mediana': f"{on_data[kpi_var].median():.4f}",
                })
            ...
            fig4.update_layout(
                title=f"{kpi_var} - Estadísticas Detalladas{ready_info}",
                height=300
            )
            charts_html.append(pio.to_html(fig4, include_plotlyjs=False, div_id=f"chart4_{kpi_var}"))
        except Exception as e:
    ...
    <body>
        <div class="header">
            <h1>{report_title}</h1>
            <p>Reporte de Análisis OPTIBAT - Generado el {datetime.now().strftime('%d/%m/%Y %H:%M')}</p>
        </div>
    ...
    html_content += f"""
        <div class="footer">
            ...
            <p><strong>Developed by Juan Cruz E.</strong> | Enhanced OPTIBAT Analysis | Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}</p>
            <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #6c757d;">
                <small>🔋 Professional OPTIBAT System Monitoring & Advanced KPI Analytics</small>
            </div>
        </div>
    </body>
    </html>
...
def main():
    ...
    <div style="background: linear-gradient(135deg, {OPTIMITIVE_COLORS['primary_red']} 0%, #CC1A2C 100%);
        ...
        <div style="margin-top: 1rem; font-size: 1rem; opacity: 0.9;">
            Professional OPTIBAT System Monitoring & Time Series Analytics
        </div>
    </div>
    ...
    if metrics: 
        ...
        with col2:
            st.metric("⚡ Eficiencia Actual", f"{metrics['efficiency_percentage']:.1f}%")
            st.metric("📊 Total Registros", f"{metrics['total_records']:,}")
        with col3:
            if metrics['breakdown']:
                st.metric("🟢 ON & Ready", f"{metrics['breakdown']['ON & Ready']:,} min")
                st.metric("🟠 OFF & Ready", f"{metrics['breakdown']['OFF & Ready (Desperdiciado)']:,} min")
    ...
    selected_vars, binary_vars, line_styles = create_variable_selector(df_filtered)
    ...
    if len(df_filtered) > 0:
        ...
        if not selected_vars:
            ...
            st.plotly_chart(create_time_series_chart(df_filtered, basic_vars, basic_binary, basic_styles), use_container_width=True)
            ...
            st.plotly_chart(create_time_series_chart(df_filtered, selected_vars, binary_vars, line_styles), use_container_width=True)
    else:
    ...
    if selected_vars:
        ...
        create_statistics_table(df_filtered, selected_kpis)
    ...
    if selected_kpis_for_detail and len(available_dfs_for_kpi) > 0:
        ...
        if on_col_for_analysis in df_for_kpi_analysis.columns:
            # Obtener columna de tiempo del DataFrame seleccionado
            time_col = None
                ...
                with st.spinner("📊 Generando reporte HTML con todos los gráficos..."):
                    try:
                        ...
                        html_content = create_html_report(
                            df_analysis=df_for_kpi_analysis,
    ...
    <div style="text-align: center; color: {OPTIMITIVE_COLORS['text_secondary']}; 
        ...
        <p><strong>Developed by Juan Cruz E.</strong> | Enhanced OPTIBAT v21 Single Button | Last Update: {datetime.now().strftime('%Y-%m-%d')}</p>
